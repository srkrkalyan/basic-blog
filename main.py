...
class Handler(webapp2.RequestHandler):
    ...
    def user_owns_blog(self, user_id, blog_id):
        ...
        print db.GqlQuery(query).get().user_id
        if int(user_id) == db.GqlQuery(
    ...
    def user_owns_comment(self, comment_id):
        if db.GqlQuery(
                ...
                str(comment_id)).get().user_id == int(
...
class BlogHandler(Handler):
    ...
    def get(self):
        ...
        if (db.GqlQuery("select * from Blog").count()) > 0:
            blogs = db.GqlQuery(
                "select * from Blog order by created desc").fetch(limit=10)
            self.render(
                "blog_front.html",
                blogs=blogs,
...
class SignUp(Handler):
    ...
    def get(self):
            # Setting a cookie in response header as soon as a request is
            # received to sign up page
    ...
    def post(self, *args, **kw):
        ...
        if username and password and (password == verify_password):
            ...
            error = "Both username and password required."\
            	   " Please give all required details"
            self.render(
                "sign_up.html",
                username=username,
                password=password,
...
class Login(Handler):
    ...
    def get(self):
        cookie_value = self.request.cookies.get('user_id')
        if cookie_value:
            if db.GqlQuery('select * from User where user_id=' +
                           str(cookie_value.split('|')[0])).get():
    ...
    # If user authentication successful, navigate to welcome page with list of
    # all his blogs
    def post(self):
        username = self.request.get("username")
        password = self.request.get("password")
        user_entry = db.GqlQuery(
            "select * from User where user_name= :1",
            username).get()
        ...
        if username and password:
            if user_entry:
                if self.validate_user_login(username, password):
                    cookie_value = str(user_entry.user_id) + '|' + \
                        str(self.hashme(str(user_entry.user_id)))
                    self.response.headers.add_header(
                        'set-cookie', 'user_id=%s; path=/' %
                        cookie_value)
                    self.redirect('/blog/welcome')
                else:
                    error = "Either user or password is incorrect."\
                            " Please check again !!!"
    ...
    def get(self):
        ...
        if user_id:
            if db.GqlQuery("select * from User where user_id =" + user_id):
                cookie_blog_id = self.request.cookies.get('blog_id')
                if cookie_blog_id:
                    blog_details = db.GqlQuery(
                        "select * from Blog where blog_id=" +
                        str(cookie_blog_id)).get()
                    self.render(
                        "new_post.html",
                        title=blog_details.title,
                        blog=blog_details.blog,
                        cancel=True)
                else:
...
class PermaLink(Handler):
    ...
    def get(self, blog_id):
        # Get the latest blog entry from Blog table using blog_id.
        # I can do this by 'select * from Blog order by created
        # desc'.fetch(limit=1)
        ...
        # Rendering Permalink page with details of title (Subject) and Blog
        # (Content)
...
class EditBlog(Handler):
    ...
    def get(self, blog_id):
        user_id = self.get_current_user()
        if user_id:
            if (db.GqlQuery("select * from Blog where blog_id=" +
                            blog_id).get().user_id) == int(user_id):
                blog = db.GqlQuery(
                    "select * from Blog where blog_id=" +
                    blog_id).get()
                self.response.headers.add_header(
...
class LikeBlog(Handler):
    ...
    def get(self, blog_id):
        ...
        if user_id:
            ...
            elif db.GqlQuery("select * from Like where user_id=" + user_id + "\
                             ""and blog_id=" + blog_id + "\
                             "" and like_flag = True").get():
                self.redirect("/blog")
            # User likes the blog
            else:
                like_entry = Like(
