...
class Handler(webapp2.RequestHandler):
	...
	def user_owns_comment(self,comment_id):
		if db.GqlQuery("select * from Comment where comment_id ="+str(comment_id)).get().user_id == int(self.get_current_user()):
			return True
		else:
			return False
...
class CommentBlog(Handler):
	...
	def post(self,blog_id):
		...
		if comment:
			comment_entry=Comment(blog_id=int(blog_id), user_id=int(user_id), 
				user_name=str(user_name), comment=comment,comment_id=comment_id)
			comment_entry.put()
...
class EditComment(Handler):
	def get(self,blog_id,comment_id):
		...
		if self.user_owns_comment(comment_id):
			...
			self.render("comment.html",blog=blog_entry,comments=comments,edit_comment=int(comment_id))
		else:
			error="You cant edit other's comments"
			blog_entry = db.GqlQuery("select * from Blog where blog_id ="+ blog_id).get()
			comments = db.GqlQuery("select * from Comment where blog_id ="+ blog_id).fetch(limit=1000)
			self.render("comment.html",blog=blog_entry,comments=comments,edit_error=error)

	def post(self,blog_id,comment_id):
		updated_comment = self.request.get("editcomment")
		comment_entry = db.GqlQuery("select * from Comment where comment_id ="+str(comment_id)).get()
